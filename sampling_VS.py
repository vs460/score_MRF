...
def shared_predictor_update_fn_T(x, t, sde, model_T, predictor, probability_flow, continuous):
  """A wrapper that configures and returns the update function of predictors."""
  score_fn = mutils.get_score_fn(sde, model_T, train=False, continuous=continuous)
  predictor_obj = predictor(sde, score_fn, probability_flow)
...
def shared_corrector_update_fn_T(x, t, sde, model_T, corrector, continuous, snr, n_steps):
  """A wrapper tha configures and returns the update function of correctors."""
  score_fn = mutils.get_score_fn(sde, model_T, train=False, continuous=continuous)
  corrector_obj = corrector(sde, score_fn, snr, n_steps)
  fn = corrector_obj.update_fn(x, t)
...
# Modified by VS to incorporate time and chemical shift dimenstion
def get_pc_fouriercs_RI(sde, predictor, corrector, inverse_scaler, snr,
  ...
  corrector_update_fn = functools.partial(shared_corrector_update_fn,
                                          ...
                                          snr=snr,
                                          n_steps=n_steps)
  predictor_update_fn_T = functools.partial(shared_predictor_update_fn_T,
                                          sde=sde,
                                          predictor=predictor,
                                          probability_flow=probability_flow,
                                          continuous=continuous)
  corrector_update_fn_T = functools.partial(shared_corrector_update_fn_T,
                                          sde=sde,
                                          corrector=corrector,
                                          continuous=continuous,
                                          snr=snr,
  ...
  def data_fidelity(mask, x, x_mean, Fy):
    kspace = fft2(x) * (1. - mask) + Fy
    # np.save('./results/single-coil/recon/smaple_img.npy', x.cpu().detach().numpy())
    x = ifft2(kspace)
    x_mean = ifft2(fft2(x_mean) * (1. - mask) + Fy)
    # np.save('./results/single-coil/recon/invivo_data_kspace.npy', kspace.cpu().detach().numpy())
    return x, x_mean
  ...
  def get_fouriercs_update_fn(update_fn):
    def fouriercs_update_fn(model, data, mask, x, t, Fy=None):
      with torch.no_grad():
        ...
        x_real = torch.real(x)
        x_imag = torch.imag(x)
        # x_T = (np.sum(np.sum(abs(x_real.cpu().numpy() + 1j * x_imag.cpu().numpy()),axis=3),axis=2))
        # x_T = np.concatenate((x_T,x_T,x_T,x_T,x_T,x_T,x_T,x_T,x_T,x_T),axis = 3)
        # x_T = torch.from_numpy(x_T).to(data.device)
        # x_T, x_mean_T = update_fn_T(x_T, vec_t, model_T=model_T)
        # x_T = x_T[0,0,0:40,0:4]
        # x_mean_T = x_mean_T[0,0,0:40,0:4]
        for j in range(40):
            for i in range(4):
                x_real_X = x_real[:,:,:,:,j,i]
                x_imag_X = x_imag[:,:,:,:,j,i]
                # perform update step with real / imag part seperately
                x_real_X, x_real_mean_X = update_fn(x_real_X, vec_t, model=model)
                x_imag_X, x_imag_mean_X = update_fn(x_imag_X, vec_t, model=model)
                x_real[0:1,0:1,:,:,j,i] = x_real_X
                x_imag[0:1,0:1,:,:,j,i] = x_imag_X
                # x_real_mean_X = x_real_X
                # x_imag_mean_X = x_imag_X
                x[0:1,0:1,:,:,j,i] = x_real[0:1,0:1,:,:,j,i] + 1j * x_imag[0:1,0:1,:,:,j,i]
                # x[0:1,0:1,:,:,j,i] = x[:,:,:,:,j,i]/torch.from_numpy(np.sum(np.sum(abs(x[:,:,:,:,j,i].cpu().numpy()),axis=3),axis=2)).to(data.device)*x_T[j,i]
                x_mean[0:1,0:1,:,:,j,i] = x_real_mean_X + 1j * x_imag_mean_X
                # x_mean[:,:,:,:,j,i] = x_mean[:,:,:,:,j,i]/torch.from_numpy(np.sum(np.sum(abs(x_mean[:,:,:,:,j,i].cpu().numpy()),axis=3),axis=2)).to(data.device)*x_mean_T[j,i]
                # data fiedelity projection   
                x_tmp, x_mean_tmp = data_fidelity(torch.squeeze(torch.squeeze(mask,dim=-1),dim=-1), x[0:1,0:1,:,:,j,i], x_mean[0:1,0:1,:,:,j,i], Fy[0:1,0:1,:,:,j,i])
                x[0:1,0:1,:,:,j,i], x_mean[0:1,0:1,:,:,j,i] = x_tmp, x_mean_tmp
  ...
  # projector_fouriercs_update_fn = get_fouriercs_update_fn(predictor_update_fn,predictor_update_fn_T)
  # corrector_fouriercs_update_fn = get_fouriercs_update_fn(corrector_update_fn,corrector_update_fn_T)
  projector_fouriercs_update_fn = get_fouriercs_update_fn(predictor_update_fn)
  corrector_fouriercs_update_fn = get_fouriercs_update_fn(corrector_update_fn)


  def pc_fouriercs(model, model_T, data, mask, Fy=None):
    with torch.no_grad():
      # Initial sample (complex-valued)
      init_sample = sde.prior_sampling(data.shape).to(data.device)
      x = ifft2(Fy + fft2(init_sample) * (1. - mask))
      np.save('./results/single-coil/recon/init_sample.npy', init_sample.cpu().detach().numpy())
      timesteps = torch.linspace(sde.T, eps, sde.N)
      for i in tqdm(range(sde.N)):
        t = timesteps[i]
        x, x_mean = corrector_fouriercs_update_fn(model, model_T, data, mask, x, t, Fy=Fy)
        x, x_mean = projector_fouriercs_update_fn(model, model_T, data, mask, x, t, Fy=Fy)
