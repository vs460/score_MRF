...
def fft2(x):
  ...
  # return torch.fft.fftshift(torch.fft.fft2(x, dim=[2,3]), dim=[2, 3])
  return torch.fft.fftshift(torch.fft.fft2(x), dim=[-1, -2])


def ifft2(x):
  """ IFFT with shifting DC to the corner of the image prior to transform"""
  # return torch.fft.ifft2(torch.fft.ifftshift(x, dim=[2, 3]), dim=[2,3])
  return torch.fft.ifft2(torch.fft.ifftshift(x, dim=[-1, -2]))


def fft2_m(x):
  """ FFT for multi-coil """
  return torch.view_as_complex(fft2c_new(torch.view_as_real(x), dim=[2,3]))
  # return torch.view_as_complex(fft2c_new(torch.view_as_real(x)))


def ifft2_m(x):
  """ IFFT for multi-coil """
  return torch.view_as_complex(ifft2c_new(torch.view_as_real(x), dim=[2,3]))
